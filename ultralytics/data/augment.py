...
EXTRA_INSTANCE_KEYS = ("body_kpts_3d", "instance_flags", "person_mask")


def _take_extra_instances(labels: dict[str, Any], index) -> None:
    """Apply an instance index selection to extra unified-schema fields."""
    for key in EXTRA_INSTANCE_KEYS:
        if key not in labels or labels[key] is None:
            continue
        if isinstance(labels[key], list):
            idx = np.asarray(index)
            if idx.dtype == bool:
                labels[key] = [v for v, keep in zip(labels[key], idx) if keep]
            else:
                labels[key] = [labels[key][int(i)] for i in idx.tolist()]
        else:
            labels[key] = labels[key][index]


def _cat_extra_instances(labels: dict[str, Any], sources: list[dict[str, Any]], indexes: list[Any] | None = None) -> None:
    """Concatenate extra unified-schema fields from source labels."""
    for key in EXTRA_INSTANCE_KEYS:
        if not any(key in s and s[key] is not None for s in sources):
            continue
        parts = []
        for i, src in enumerate(sources):
            ...
            if value is None:
                continue
            index = None if indexes is None else indexes[i]
            if isinstance(value, list):
                if index is None:
                    part = value
                else:
                    idx = np.asarray(index)
                    part = [v for v, keep in zip(value, idx) if keep] if idx.dtype == bool else [value[int(j)] for j in idx.tolist()]
                parts.extend(part)
            else:
                parts.append(value if index is None else value[index])
        if parts:
...
class Mosaic(BaseMixTransform):
    ...
    def __init__(self, dataset, imgsz: int | list[int] | tuple[int, int] = 640, p: float = 1.0, n: int = 4):
        ...
        self.imgsz = img_size_hw(imgsz)
        self.border = (-self.imgsz[0] // 2, -self.imgsz[1] // 2)  # height, width
        self.n = n
    ...
    def get_params(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        h_out, w_out = self.imgsz
        layout = []
        if self.n == 4:
            yc = int(random.uniform(-self.border[0], 2 * h_out + self.border[0]))
            xc = int(random.uniform(-self.border[1], 2 * w_out + self.border[1]))
            for i in range(4):
                ...
                if i == 0:  # top left
                    ...
                    x1a, y1a, x2a, y2a = max(xc - w, 0), yc, xc, min(h_out * 2, yc + h)
                    x1b, y1b, x2b, y2b = w - (x2a - x1a), 0, w, min(y2a - y1a, h)
                elif i == 3:  # bottom right
                    x1a, y1a, x2a, y2a = xc, yc, min(xc + w, w_out * 2), min(h_out * 2, yc + h)
            ...
            for i in range(9):
                ...
                h, w = labels_patch.get("resized_shape", img.shape[:2])
                if i == 0:  # center
                    c = w_out, h_out, w_out + w, h_out + h
                    h0, w0 = h, w
                elif i == 1:  # top
                    c = w_out, h_out - h, w_out + w, h_out
                elif i == 2:  # top right
                    c = w_out + wp, h_out - h, w_out + wp + w, h_out
                elif i == 3:  # right
                    c = w_out + w0, h_out, w_out + w0 + w, h_out + h
                elif i == 4:  # bottom right
                    c = w_out + w0, h_out + hp, w_out + w0 + w, h_out + hp + h
                elif i == 5:  # bottom
                    c = w_out + w0 - w, h_out + h0, w_out + w0, h_out + h0 + h
                elif i == 6:  # bottom left
                    c = w_out + w0 - wp - w, h_out + h0, w_out + w0 - wp, h_out + h0 + h
                elif i == 7:  # left
                    c = w_out - w, h_out + h0 - h, w_out, h_out + h0
                elif i == 8:  # top left
                    c = w_out - w, h_out + h0 - hp - h, w_out, h_out + h0 - hp
                padw, padh = c[:2]
    ...
    def apply_image(self, labels: dict[str, Any], params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        if self.n == 4:
            img4 = np.full((h_out * 2, w_out * 2, labels["img"].shape[2]), 114, dtype=np.uint8)
            for item in layout:
            ...
            img9 = np.full((h_out * 3, w_out * 3, labels["img"].shape[2]), 114, dtype=np.uint8)
            for item in layout:
                labels_patch = item["labels_patch"]
                img = labels_patch["img"]
    ...
    def apply_instances(self, labels: dict[str, Any], params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        for item in layout:
            if self.n == 4:
                ...
                padw = item["padw"] + self.border[1]
                padh = item["padh"] + self.border[0]
    ...
    def _cat_labels(self, mosaic_labels: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        cls = []
        instances = []
        img_h, img_w = self.imgsz[0] * 2, self.imgsz[1] * 2  # mosaic imgsz
        ...
        final_labels = {
            ...
            "resized_shape": (img_h, img_w),
            "cls": np.concatenate(cls, 0),
            "instances": Instances.concatenate(instances, axis=0),
            "mosaic_border": self.border,
        }
        final_labels["instances"].clip(img_w, img_h)
...
class RandomPerspective(BaseTransform):
    ...
    def apply_instances(self, labels: dict[str, Any], params: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        labels["cls"] = cls[i]
        _take_extra_instances(labels, i)
        return labels
...
class RandomFlip(BaseTransform):
    ...
    def apply_image(self, labels: dict[str, Any], params: dict[str, Any]) -> dict[str, Any]:
        ...
        if params["flip"] and labels.get("scene_mask") is not None:
            if params["direction"] == "vertical":
                labels["scene_mask"] = np.ascontiguousarray(np.flipud(labels["scene_mask"]))
            elif params["direction"] == "horizontal":
                labels["scene_mask"] = np.ascontiguousarray(np.fliplr(labels["scene_mask"]))
        return labels
        ...
        if params["flip"]:
            ...
            if params["flip_idx"] is not None and instances.keypoints is not None:
                ...
                if "body_kpts_3d" in labels and labels["body_kpts_3d"] is not None:
                    labels["body_kpts_3d"] = np.ascontiguousarray(labels["body_kpts_3d"][:, params["flip_idx"], :])
...
class Format(BaseTransform):
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.return_mask:
            if nl:
                ...
                if "body_kpts_3d" in labels and labels["body_kpts_3d"] is not None and hasattr(self, "_last_segment_idx"):
                    labels["body_kpts_3d"] = labels["body_kpts_3d"][self._last_segment_idx]
                if "instance_flags" in labels and labels["instance_flags"] is not None and hasattr(self, "_last_segment_idx"):
                    labels["instance_flags"] = labels["instance_flags"][self._last_segment_idx]
                if "person_mask" in labels and labels["person_mask"] is not None and hasattr(self, "_last_segment_idx"):
                    labels["person_mask"] = [labels["person_mask"][int(i)] for i in self._last_segment_idx.tolist()]
        ...
        if self.return_keypoint:
            labels["keypoints"] = (
                torch.empty((0, 0, 3)) if instances.keypoints is None else torch.from_numpy(instances.keypoints)
            )
            if self.normalize:
        ...
        if labels.get("scene_mask") is not None:
            ...
            if scene_mask.shape[-2:] != (img.shape[0] // self.mask_ratio, img.shape[1] // self.mask_ratio):
                scene_mask = F.interpolate(
                    scene_mask[None, None].float(),
                    (img.shape[0] // self.mask_ratio, img.shape[1] // self.mask_ratio),
                    mode="nearest",
                )[0, 0].long()
    ...
    def _format_segments(
        ...
        """
        segments = instances.segments
        self._last_segment_idx = np.arange(len(segments))
        if self.mask_overlap:
            masks, sorted_idx = polygons2masks_overlap((h, w), segments, downsample_ratio=self.mask_ratio)
            masks = masks[None]  # (640, 640) -> (1, 640, 640)
            instances = instances[sorted_idx]
            cls = cls[sorted_idx]
            self._last_segment_idx = sorted_idx
        else:
...
def v8_transforms(dataset, imgsz: int | list[int] | tuple[int, int], hyp: IterableSimpleNamespace, stretch: bool = False):
    ...
    Examples:
        ...
        >>> transforms = v8_transforms(dataset, imgsz=640, hyp=hyp)
    """
    imgsz_hw = img_size_hw(imgsz)
    mosaic = Mosaic(dataset, imgsz=imgsz_hw, p=hyp.mosaic)
    affine = RandomPerspective(
        ...
        pre_transform=None if stretch else LetterBox(new_shape=imgsz_hw),
    )
    ...
    if hyp.copy_paste_mode == "flip":
        ...
        pre_transform.append(
            CopyPaste(
                dataset,
                pre_transform=Compose([Mosaic(dataset, imgsz=imgsz_hw, p=hyp.mosaic), affine]),
                p=hyp.copy_paste,
